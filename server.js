...
const REGISTRY_FILE = path.join(__dirname, "players.json");
...
function applyPlayerRegistry(matrix) {
  if (!Array.isArray(matrix) || matrix.length < 2) return matrix;
  const header = matrix[0];
  const idIndex = header.findIndex(h => /^(player id|uid)$/i.test(clean(h)));
  const nameIndex = header.findIndex(h => /^(player name|nickname)$/i.test(clean(h)));
  if (idIndex < 0 || nameIndex < 0) return matrix;
  return [header, ...matrix.slice(1).map(row => {
    const copy = Array.isArray(row) ? row.slice() : row;
    if (!Array.isArray(copy)) return copy;
    const id = clean(copy[idIndex]);
    if (id && playerRegistry[id]) copy[nameIndex] = playerRegistry[id].nickname;
    return copy;
  })];
}

function applyRegistryToResult(data) {
  if (!data) return data;
  return { ...data, playerData: applyPlayerRegistry(data.playerData || []) };
}
  ...
  try {
    ...
    if (team.length >= 2 || player.length >= 2) {
      lastResult = {
        ...
        playerData: applyPlayerRegistry(player.length >= 2 ? player : lastResult.playerData)
      };
    }
...
async function capture(matchId) {
  ...
  const result = {
    ...
    capturedAt: new Date().toISOString(),
    teamData: team,
    playerData: applyPlayerRegistry(player)
  };
...
app.get("/api/players", (req, res) => {
  const players = Object.entries(playerRegistry)
    .map(([playerId, value]) => ({ playerId, nickname: value.nickname }))
    .sort((a, b) => a.playerId.localeCompare(b.playerId, "pt-BR", { numeric: true }));
...
app.post("/api/players", (req, res) => {
  ...
  if (!/^\d+$/.test(playerId)) return res.status(400).json({ ok:false, error:"O ID do jogador deve conter apenas números." });
  if (!nickname) return res.status(400).json({ ok:false, error:"Informe o Nick registrado." });
  playerRegistry[playerId] = { nickname };
  savePlayerRegistry(playerRegistry);
  if (lastResult) lastResult = applyRegistryToResult(lastResult);
  res.json({ ok:true, player:{ playerId, nickname } });
});

app.delete("/api/players/:playerId", (req, res) => {
  const playerId = clean(req.params.playerId);
  if (!Object.prototype.hasOwnProperty.call(playerRegistry, playerId)) return res.status(404).json({ ok:false, error:"Jogador não encontrado." });
